...
+    var hadPrev = lastHandled.has(key);
+    var prev = hadPrev ? lastHandled.get(key) : 0;
+    if (hadPrev) {
+      var delta = now - prev;
+      if (delta >= 0 && delta < DUP_WINDOW) {
+        // Duplicate within the window → block while keeping the last timestamp
